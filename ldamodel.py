...
from utils import seconds2clock

logger = logging.getLogger(__name__)


"""
Replace visdom logger with tensorboard logger.
"""


class Callback():
    def __init__(self, metrics, log_dir):
    ...
    def on_epoch_end(self, epoch, topics=None):
        ...
        for metric in self.metrics:
            label = str(metric)
            value = metric.get_value(topics=topics, model=self.model)
            current_metrics[label] = value
            self.tb_logger.add_scalar(label, value, epoch)
        for label, value in current_metrics.items():
            logger.info(f"{label:15s}: {value:.4f}")
...
class LdaModel(interfaces.TransformationABC, basemodel.BaseTopicModel):
    ...
    def update_alpha(self, gammat, rho):
        ...
        logger.debug("optimized alpha %s", list(self.alpha))
        ...
        for pass_ in range(passes):
            ...
            for chunk_no, chunk in enumerate(chunks):
                ...
                if self.dispatcher:
                    ...
                    logger.info(
                        "PROGRESS: dispatching documents up to #%i/%i",
            ...
            logger.info(f'Epoch duration: {seconds2clock(elapse)}.')
            logger.info(f'Save model to {self.model_dir}')
    ...
    def do_mstep(self, rho, other, extra_pass=False):
        ...
        logger.info(f"{'topic diff':15s}: {diff:.4f}")
        logger.info(f"{'rho':15s}: {rho:.4f}")
    ...
    def show_topics(self, num_topics=10, num_words=10, log=False, formatted=True):
        ...
        for i in chosen_topics:
            ...
            if log:
                logger.debug("topic #%i (%.3f): %s", i, self.alpha[i], topic_)
